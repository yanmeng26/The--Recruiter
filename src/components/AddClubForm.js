...
class AddClubForm extends Component {
    constructor() {
        super();
        this.state = {
            recruits: [],
            club_id: null,
            selectvalue: "",
            team: {"team_id":null, "type":"Club", "team_name":"", "school_id":null, "club_id":"", "school_club_name":"", "street_address_1":"", "street_address_2":"", "city":"", "state":"", "zip":"","update_secondary":false},
            schoolClub: {},
    ...
    handleSubmit(e) {
        ...
        if ((this.state.showAdd && (props.team_name != "" && props.school_club_name != "" && props.street_address_1 !== "" && props.city !== "" && props.state !== "" && props.city !== "" && props.zip !== "")) || (!this.state.showAdd && this.state.club_id)) {
            e.preventDefault();
            if (this.state.club_id === "other-value") 
                this.postData(this.state.team, "/endpoints/add-club-or-school", true);
            else if (this.state.updateRecruit)
                this.postData({"club_id":this.state.club_id,"recruit_id":this.props.recruit_id}, "/endpoints/add-team-recruit", false);
            else if (this.state.updateCoach)
                this.postData({"club_id":this.state.club_id,"coach_id":this.props.coach_id,"type":"Club"}, "/endpoints/add-team-coach", false);
        }
    }
        ...
        .then(function(response) {
            if (response.errno) {
                ...
                if (this.state.updateRecruit)
                    this.postData({"club_id":data.club_id,"recruit_id":this.props.recruit_id}, "/endpoints/add-team-recruit", false);
                else if (this.state.updateCoach)
                    this.postData({"club_id":data.club_id,"coach_id":this.props.coach_id,"type":"Club"}, "/endpoints/add-team-coach", false);
                
            }
            else {
                if (this.props.add_recruit)
                    alert("Club has been added to this recruit.");
                else
    ...
    render() {
        let query = "";
        if (this.props.recruit_id) {
            query = "?not_recruit_id="+this.props.recruit_id;
        }
        else if (this.props.coach_id) {
        ...
        return (
            <form>
                {this.props.showDropdown && <div className="form-group">
                    ...
                    <DropdownClub inputId="club_id" inputFor="club_id" query={query} value={this.state.club_id} onChangeValue={(e) => this.updateClub(e)} required={true} other={this.props.other}/>
                </div>}
                {this.state.showAdd && <div><div>
                    ...
                    <div className="form-group">
                        <label htmlFor="school_club_name">Club Name: <span className="required">*</span> </label>
